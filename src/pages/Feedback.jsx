...
const Feedback = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (feedbackType === "event" && !selectedEvent) {
      alert("Please select an event to provide feedback for.");
      return;
    }
    ...
    if (!isAnonymous && (!formData.name.trim() || !formData.email.trim())) {
      alert("Please provide your name and email for verified feedback.");
      return;
    }

    if (feedbackType === "event" && rating === 0) {
      alert("Please provide a rating for the event.");
      return;
    }
    ...
    try {
      ...
      const feedbackData = {
        ...
        event: selectedEvent,
        eventTitle: recentEvents.find(e => e.id == selectedEvent)?.title,
        rating: rating,
        ...formData,
      ...
      alert("Thank you for your feedback! Your input helps us improve campus experiences.");
